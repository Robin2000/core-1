...
bool ChatHandler::HandleAddItemCommand(char* args)
{
    ...
    if (!ExtractUInt32(&cId, itemId))                       // [name] manual form
    {
		...
		QueryResult *result = WorldDatabase.PQuery("SELECT A.entry FROM item_template A LEFT JOIN locales_item B ON A.entry=B.entry WHERE A.name = '%s' or B.name_loc4 = '%s'", itemName.c_str(), itemName.c_str());
        if (!result)
        {
